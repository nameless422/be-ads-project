...
func (s *Service) HandleJob(ctx context.Context, job messagingdomain.CollectJob) error {
	...
	stored, err := s.rawRepository.SaveBatch(ctx, rawdomain.SaveBatchInput{
		...
		SourceWatermark:   nextWatermarkValue(batch.NextWatermark, job.WatermarkValue),
		CollectedAt:       batch.CollectedAtUTC.Format(time.RFC3339),
		Checkpoint: rawdomain.SyncCheckpoint{
			ProfileID:       job.ProfileID,
			NextCursor:      batch.NextCursor,
			NextWatermark:   nextWatermarkValue(batch.NextWatermark, job.WatermarkValue),
			LastCollectedAt: batch.CollectedAtUTC,
		},
...
func nextWatermarkValue(next *time.Time, fallback string) string {
	if next == nil {
		return fallback
	}
